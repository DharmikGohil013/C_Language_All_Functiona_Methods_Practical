                    ...
                    case 2:
                     ...
                     case 3:
                        ...
                        switch(i5)
                        {
                            ...
                            cout<<"\n\t\tEnter Date : - ";
                            cin>>date111;
                            show_daily(date111,num11,pass11);
                            break;
                            case 0:
                            cout<<"THANKYOU";
                            goto rep111;
...
void daily_dairy(int n1, int p1) {
    ...
    fstream output2("hostel_daily.txt", ios::binary | ios::in | ios::out);
...
hostel ac;
    ...
    input.open("hostel_daily.txt", ios::binary | ios::app);
    if (!input.is_open())
    {
        cout << "Error: Could not open file for writing." << endl;
        return;
    }
    if (!input.eof() && found111 == false)
    {
        input.read(reinterpret_cast<char *>(&ac),sizeof(hostel));
        if (ac.retacno1() == a1)
        {
            if(ac.retpassword()==p1)
            {
       ...
       if(ac.retdailydate1()==d1)
       {
            ...
            //input.read(reinterpret_cast<char *>(&ac),sizeof(hostel));
             input.write(reinterpret_cast<char *>(&ac),sizeof(hostel));
            ac.show_daily2();
            found111=true;
       }
       else{
...
{
    hostel ac;
    fstream output;//output obj is use hostel.txt
    //output is use hostel_txt
   fstream output2;

    bool found11=false;
    //output.open("hostel_daily.txt", ios::binary | ios::in | ios::out);
    output.open("hostel.txt", ios::binary | ios::in | ios::out);
    output2.open("hostel_daily.txt",ios::binary | ios::in | ios::out) ;
    if (!output.is_open())
    {
        cout << "\n\t\tError: Could not open file for writing. read1" << endl;
        return;
    }
    if(!output2.is_open())
    {
        cout << "\n\t\tError: Could not open file for writing. read2" << endl;
        return;
    }
     while (!output2.eof() && found11 == false && !output.eof())
     {
         output.read(reinterpret_cast<char *>(&ac), sizeof(hostel));
       if (ac.retacno1() == n1)
